...
class MiniMaxProvider(ImageProvider):
    ...
    # MiniMax uses aspect_ratio instead of WxH size strings
    ASPECT_MAP = {
        "cover": "21:9",
        "article": "16:9",
        "vertical": "9:16",
    ...
    def resolve_size(self, preset: str) -> str:
        if preset in self.ASPECT_MAP:
            return self.ASPECT_MAP[preset]
        return preset
    ...
    def generate(self, prompt: str, size: str) -> bytes:
        ...
        if base_resp.get("status_code") != 0:
            msg = base_resp.get("status_msg", json.dumps(data, ensure_ascii=False))
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate images using AI (doubao-seedream, OpenAI DALL-E, MiniMax image-01, etc.)"
    )
    ...
    parser.add_argument(
        "--provider",
        default=None,
        help="Override provider (doubao, openai, minimax). Default: from config.yaml",
    )
