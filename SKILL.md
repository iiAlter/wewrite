...
**优先使用 TrendRadar MCP**（深度 AI 分析，含舆论情绪、异动信号、趋势预判）：

```bash
python3 {skill_dir}/../trendradar-content/trendradar_content.py --date {今天YYYY-MM-DD} --platform gzh
```

脚本调用 TrendRadar MCP（`http://127.0.0.1:3333/mcp`），返回结构化分析：
- **核心热点**：TOP 话题列表（含出现频次）
- **舆论情绪**：正面/负面/中性 + 置信度
- **异动信号**：异常波动关键词
- **趋势预判**：后续推演

从中提取热点列表，为每条标注所属领域和可创作性评分（1-10）。
